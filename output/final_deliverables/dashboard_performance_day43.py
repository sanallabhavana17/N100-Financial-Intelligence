...
if __name__ == "__main__":
    ...
    overall_avg = statistics.mean(result["avg_ms"] for result in results)

    worst = max(results, key=lambda result: result["avg_ms"])

    print("\n" + "-" * 90)
    print(f"Overall average: {overall_avg:.2f} ms")
    print(f"Worst ticker: {worst['ticker']} " f"({worst['avg_ms']:.2f} ms)")

    print("\nAcceptance target: Company Profile < 3 seconds")
    print("Result: PASS" if worst["avg_ms"] < 3000 else "Result: REVIEW REQUIRED")
