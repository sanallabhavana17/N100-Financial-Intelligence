...
def test_roe_normal():
    result = return_on_equity(net_profit=100, equity_capital=200, reserves=300)

    assert result == 20


def test_roe_negative_equity():
    result = return_on_equity(net_profit=100, equity_capital=-500, reserves=100)

    assert result is None


def test_roa_zero_assets():
    result = return_on_assets(net_profit=100, total_assets=0)
