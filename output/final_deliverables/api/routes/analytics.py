...
def screener(
    ...
    try:
        ...
        if min_roe is not None:
            conditions.append("CAST(fr.return_on_equity_pct AS REAL) >= ?")
            params.append(min_roe)

        if max_de is not None:
            conditions.append(
                """(
                    LOWER(COALESCE(s.broad_sector, '')) = 'financials'
                    OR CAST(fr.debt_to_equity AS REAL) <= ?
                )"""
            )
            params.append(max_de)

        if min_fcf is not None:
            conditions.append("CAST(fr.free_cash_flow_cr AS REAL) >= ?")
...
@router.get("/sectors")
def get_sectors():
    """Return all broad sectors with company counts and median financial metrics."""
    conn = get_connection()
    ...
    try:
        rows = conn.execute(
            ...
            SELECT
                ...
                CAST(fr.return_on_equity_pct AS REAL) AS roe,
                CAST(fr.debt_to_equity AS REAL) AS debt_to_equity,
                CAST(mc.pe_ratio AS REAL) AS pe
            FROM sectors s
            ...
            result.append(
                {
                    ...
                    "median_roe": round(group["roe"].dropna().median(), 2),
                    "median_pe": round(group["pe"].dropna().median(), 2),
                    "median_de": round(
                        group["debt_to_equity"].dropna().median(), 2
                    ),
                }
            )
            ...
            "documents": [
                {
                    **dict(row),
                    "is_url_valid": bool(
                        row["annual_report"]
                        and str(row["annual_report"]).strip().lower() not in {"null", "none"}
                        and str(row["annual_report"]).strip().startswith(("http://", "https://"))
                    ),
                }
                for row in rows
            ],
...
def get_portfolio_stats():
    ...
    for _, row in df.iterrows():
        group = row["statistic_group"]
        metric = row["metric"]
        statistic = row["statistic"]
        value = row["value"]

        if group == "Portfolio KPI":
            portfolio_kpis.setdefault(metric, {})[statistic] = float(value)

        elif group == "Sector Distribution":
            sector_distribution[metric] = int(value)

        elif group == "Market Cap Distribution":
            market_cap_distribution[metric] = int(value)
